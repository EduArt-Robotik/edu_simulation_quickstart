if [ -z "$TMUX" ]; then
  ...
  tmux send-keys "PYTHONPATH=/home/user/python_env/.flet/lib/python3.12/site-packages:$PYTHONPATH ros2 run edu_virtual_joy virtual_joy --ros-args -r __ns:=/eduard" C-m
  ...
  tmux send-keys "ros2 launch /home/user/workspace/ros2_ws/src/simulation/launch/robot_monitor.launch.py edu_robot_namespace:=eduard" C-m
